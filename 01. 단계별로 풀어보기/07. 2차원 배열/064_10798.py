...
max_len = max(len(string_list[i]) for i in range(5))

answer = []
for i in range(max_len):
    for j in range(5):
        try:
            answer += string_list[j][i]
        except:
            pass
...
max_len = max(len(row) for row in string_list)

answer = []
for i in range(max_len):
    for j in range(5):
        if i < len(string_list[j]): # try-except 보단 if문을.
            answer.append(string_list[j][i]) # + 보단 append를.

print(*answer, sep='')
'''
# 더 더 최적화해볼까???

string_list = [input() for i in range(5)] # 그냥 문자열로 받아서 리스트를 만든다

for i in range(15): # 어차피 15자가 최대니까, max_len도 빼버려?
    for j in range(5):
        if i < len(string_list[j]): # 파이썬은 문자열도 인덱싱이 된다.
            print(string_list[j][i], end="") # 굳이 .append() 안하고 바로 print()
