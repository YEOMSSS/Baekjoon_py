...
for x in range(-999, 1000):
    for y in range(-999, 1000):
        if (nums[0] * x + nums[1] * y == nums[2] and
            nums[3] * x + nums[4] * y == nums[5]):
            print(x, y)

# 뭐, 이정도는 1억개도 안되는데 1초 안으로 계산 끝나지 않겠어?
'''
# 맞긴 했는데, 수학적으로는 이걸 어떻게 풀까?
# 행렬에서 크레머의 공식을 이용한다.
# 해는 유일하며, x = (ce - bf) / (ae - bd), y = (af - cd) / (ae - bd)
# 해가 하나라는 조건이 있으니 사용 가능.

a, b, c, d, e, f = list(map(int, input().split()))

denom = a*e - b*d # denominator는 분모를 뜻한다.
# ㅣa bㅣ의 행렬식이 a*e - b*d이다.
# ㅣd eㅣ행렬식이 0이 아니면 해는 하나뿐이다.
# 2x+3y=4 얘들을 보면 저게 왜 성립하는지 알 수 있다.
# 4x+6y=8 2*6 == 3*4 사실상 같은 직선. 무한히 많은 해.
x = (c*e - b*f) // denom
y = (a*f - c*d) // denom
