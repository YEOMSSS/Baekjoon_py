...
은메달 수상자는 1번 국가의 2번 학생이며, 동메달 수상자는 3번 국가의 4번 학생이다.
...
student_scores = [list(map(int, input().split())) for _ in range(N)]

first, second, third = [0, 0, -1], [0, 0, -1], [0, 0, -1]
for score in student_scores:
    if score[2] > first[2]:
        third = second
        second = first
        first = score
    elif score[2] > second[2]:
        third = second
        second = score
    elif score[2] > third[2]:
        third = score
...
def main():
    ...
    # x[2] 기준으로 내림차순 정렬
    student_scores.sort(key= lambda x: -x[2])

    print(*student_scores[0][: 2])
    print(*student_scores[1][: 2])

    if student_scores[0][0] == student_scores[1][0]:
        for score in student_scores:
            if score[0] != student_scores[0][0]:
                print(*score[: 2])
                return
    else:
        print(*student_scores[2][: 2])
