...
#     basket_list.append(0) # 0으로 이루어진 리스트 생성

basket_list = [0] * basket_num # 위보다 이게 훨씬 낫네

for i in range(count):
    start, end, ball_num = map(int, input().split())
    for j in range(start - 1, end):
        basket_list[j] = ball_num # 0을 ball_num으로 바꿈

print(*basket_list) # 이렇게 쓰면 공백으로 구분되어 출력됨
