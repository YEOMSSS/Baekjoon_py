...
# enumerate를 사용하지 않은 방법
# for i in range(length):
#     deci += nums[length - 1 - i] * (A ** i)

# sum을 사용하지 않은 방법
# for i, num in enumerate(nums[::-1]): # A진법을 10진법으로 변환
#     deci += num * (A ** i) # A진수의 뒷자리부터 계산해서 더해준다.

# sum을 이용한 한 줄 코드
deci = sum(num * (A ** i) for i, num in enumerate(nums[::-1]))

answer = []
while deci != 0:            # 10진법을 B진법으로 변환
    answer.append(deci % B) # 나머지를 구해 현재 자릿수에 넣고
    deci //= B              # 다음 자릿수를 위해 값을 갱신한다.

print(" ".join(map(str, answer[::-1]))) # 높은 자릿수부터 출력