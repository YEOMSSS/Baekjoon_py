...
for i in range(1, num + 1):
    for j in range(1, i + 1):
        ...
        # 그러니까 타일이랑 똑같은 거다. 마지막 타일이 카드팩이라 생각하면 된다.
...
# 왜 그리디가 틀렸는지에 대한 답변을 받았다.
# 3
# 1 7 10일 때
# 카드당 가격이 제일 비싼 건 7이어서
# 7 + 1 = 8을 내놓지만..
# 실제로는 10을 구매해야 한다는 것이다.

# 그리디 알고리즘으로 풀었는데 틀린다는건 코딩과정에서 실수가 있었거나
# 지역최적해가 전역최적해를 보장하지 않는다는걸 뜻합니다.

# 라는 답변을 얻었다.
# 현 단계에서 최적인 것을 골라도, 그것이 전체에서 최적인 게 아니다.
# 그래서 이 문제는 dp로 풀어야 한다는 것이다.