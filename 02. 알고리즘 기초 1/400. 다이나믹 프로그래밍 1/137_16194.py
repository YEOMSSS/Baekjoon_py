...
P1 = 1, P2 = 5, P3 = 6, P4 = 7인 경우에 민규가 카드 4개를 갖기 위해 지불해야 하는 금액의 최솟값은 4원이다.
...
# 자, 똑같은 문제다. 그런데 이번엔 최솟값을 구해보는 것이다.
# 그냥 똑같이 풀면 되는데, min을 사용하면 되겠지.
'''
num = int(input())
...
for i in range(2, num + 1):
    for j in range(1, i + 1):
        dp[i] = min(dp[i], dp[i - j] + pack_prices[j - 1])

print(dp[num])
'''
# 그냥 max를 min으로 바꿔서는 dp의 초기값만 계속 나온다.
num = int(input())

pack_prices = list(map(int, input().split()))
dp = [0] + pack_prices # 초기값에 팩가격을 넣는다.

for i in range(1, num + 1):
    for j in range(1, i): # i+1이 아니어도 되는게, 어차피 이미 테이블이 카드팩 가격이다.
        # dp[i-j]가 0일때를 굳이 생각할 필요가 없는 것이다.
        dp[i] = min(dp[i], dp[i - j] + pack_prices[j - 1])
