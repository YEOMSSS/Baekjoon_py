...
# 아이디어는... 한 층의 둘레의 길이는 정사각형의 세 변의 길이의 합과 같다.
# 층마다 한 변 * 3 을 더해준 후 마지막에 한 변 * 층수 를 곱해준다.
# 한 변의 길이는 1. 식은 1 * 3 * 층수 + 1 * 층수. 정리하면 4 * 층수.
'''
floor = int(input())

answer = 1 * 3 * floor + 1 * floor
...
# 결국 정사각형의 둘레의 길이인 것을 알 수 있다.
# 둘레들을 하나하나 분해해서 밑변이 기준이 되는 정사각형의 변으로 밀어보면
# 결국 밑변을 기준으로 한 정사각형이 되는 것을 알 수 있다.